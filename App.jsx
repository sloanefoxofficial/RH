...
export default function App() {
  ...
  goRef.current = go;
  const DEEP_LINK_SCREENS = ["notifications", "coordinator", "admin"];
  useEffect(() => {
...
function CoordinatorChat({ session, onBack }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      fetch("/api/push", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ toAdmins: true, title: `New message from ${session.user.email || "a member"}`, body: text, target: "admin", url: "/?open=admin" }),
      }).catch(() => {});
