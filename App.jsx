...
export default function App() {
  ...
  const saveSettings = useCallback(({ textScale: ts, reduceMotion: rm, bargeIn: bi, responseSpeed: rs }) => {
    ...
    (async () => {
      ...
      try {
        const { data } = await supabase.from("member_data")
          .select("profile,answers,plan,progress,journal").eq("user_id", session.user.id).single();
        if (data) {
    ...
    <div style={{ minHeight: "100vh", color: T.ink,
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "0 16px 132px", position: "relative", zIndex: 1, zoom: textScale }}>
        {authEnabled && !authChecked ? (
          ...
          <ProgramInfo onBack={back} onMessageJuan={() => go("coordinator")} onRequestAppointment={() => go("appointmentRequest")} />
        ) : screen === "appointmentRequest" ? (
          <AppointmentRequest session={session} onBack={back} />
        ) : screen === "memory" ? (
          <MemoryManager memories={memories} memoryOn={memoryOn}
            onSave={(list, on) => saveMemories(list, on)} onBack={back} />
        ) : screen === "settings" ? (
          <Settings textScale={textScale} reduceMotion={reduceMotion} bargeIn={bargeIn} responseSpeed={responseSpeed}
            session={session} authEnabled={showAuth} onSave={saveSettings} onBack={back}
            onOpenBugReport={() => go("bugReport")} />
        ) : screen === "bugReport" ? (
...
function Brand({ right }) {
  return (
    <header style={{ padding: "18px 2px 8px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ lineHeight: 1.15, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: T.sub, marginTop: 2, whiteSpace: "nowrap" }}>You never have to walk it alone</div>
        </div>
      </div>
      {right && (
...
const nevernSearchUrl = (q) => NEVERN_CHANNEL_URL + "/search?query=" + encodeURIComponent(q);
const NEVERN_VIDEOS = [
  { title: "Breathing technique #1", url: "https://youtube.com/shorts/bkgigQ9MLIE", tags: ["breathing", "breathe", "calm", "stress", "anxiety", "burnout"] },
  { title: "Breathing technique #2", url: "https://youtube.com/shorts/GKZSqgHQW2Y", tags: ["breathing", "breathe", "calm", "stress", "anxiety", "burnout"] },
  { title: "Breathing technique #3", url: "https://youtube.com/shorts/K0-Wsiolkns", tags: ["breathing", "breathe", "calm", "stress", "anxiety", "burnout"] },
  { title: "Breathing technique #4", url: "https://youtube.com/shorts/tOJBcJyn36M", tags: ["breathing", "breathe", "calm", "stress", "anxiety", "burnout"] },
];

function NevernSpotlight() {
  const [q, setQ] = useState("");
  const query = q.trim().toLowerCase();
  const matches = query
    ? NEVERN_VIDEOS.filter((v) => v.title.toLowerCase().includes(query) || v.tags.some((t) => t.includes(query) || query.includes(t)))
    : NEVERN_VIDEOS;

  return (
    <div style={{ background: T.card, borderRadius: 20, padding: 16, boxShadow: T.soft, marginBottom: 18 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
        <div style={{ width: 46, height: 46, borderRadius: "50%", background: "#fbe1e1", display: "grid", placeItems: "center",
          ...
          <Youtube size={22} color="#cf5147" style={{ display: "none" }} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: 15.5 }}>Nevern | Psychologist</div>
          <div style={{ fontSize: 12.5, color: T.sub }}>Helping you move from stress and burnout</div>
        </div>
      ...
      <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search his videos — e.g. breathing, stress, anxiety"
        style={{ ...inputStyle, marginBottom: 10 }} />

      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {matches.map((v) => (
          <a key={v.url} href={v.url} target="_blank" rel="noopener noreferrer"
            style={{ display: "flex", alignItems: "center", gap: 12, background: "#faf7fc", borderRadius: 14, padding: 12,
              textDecoration: "none", color: T.ink }}>
            <div style={{ width: 34, height: 34, borderRadius: 10, background: "#fbe1e1", display: "grid", placeItems: "center", flexShrink: 0 }}>
              <Play size={15} color="#cf5147" />
            </div>
            <span style={{ flex: 1, fontSize: 13.5, fontWeight: 600 }}>{v.title}</span>
            <ExternalLink size={15} color={T.sub} />
          </a>
        ))}
        {matches.length === 0 && (
          <a href={nevernSearchUrl(q)} target="_blank" rel="noopener noreferrer"
            style={{ display: "flex", alignItems: "center", gap: 12, background: "#faf7fc", borderRadius: 14, padding: 12,
              textDecoration: "none", color: T.ink }}>
            <div style={{ width: 34, height: 34, borderRadius: 10, background: "#fbe1e1", display: "grid", placeItems: "center", flexShrink: 0 }}>
              <Search size={15} color="#cf5147" />
            </div>
            <span style={{ flex: 1, fontSize: 13.5, fontWeight: 600 }}>Search "{q}" on his channel</span>
            <ExternalLink size={15} color={T.sub} />
          </a>
        )}
      </div>
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {rows && rows.length === 0 && <div style={{ color: T.sub, fontSize: 14, padding: "6px 2px" }}>No requests yet.</div>}
        {(rows || []).map((r) => (
...
function AdminBugReports({ onBack }) {
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {rows === null && <div style={{ color: T.sub, fontSize: 14, padding: "6px 2px" }}>Loading…</div>}
        {rows && rows.length === 0 && <div style={{ color: T.sub, fontSize: 14, padding: "6px 2px" }}>No bug reports yet.</div>}
        {(rows || []).map((r) => (
          <div key={r.id} style={{ background: T.card, borderRadius: 16, padding: 14, boxShadow: T.soft }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8, marginBottom: 6 }}>
            ...
            <div style={{ fontSize: 14, lineHeight: 1.5, whiteSpace: "pre-wrap" }}>{r.description}</div>
          </div>
        ))}
      </div>
    </>
...
function Settings({ textScale, reduceMotion, bargeIn, responseSpeed, session, authEnabled, onSave, onBack, onOpenBugReport }) {
...
const hubIconBtn = (color) => ({ background: "#fff", border: `1px solid ${T.line}`, borderRadius: 999, width: 42, height: 42,
...
function AppointmentRequest({ session, onBack }) {
  ...
  const canSubmit = name.trim() && contact.trim() && (anyTime || preferredTime.trim());
  ...
  const submit = async () => {
    ...
    try {
      const { error } = await supabase.from("appointment_requests").insert({
        ...
        any_time: anyTime,
        preferred_time: anyTime ? null : preferredTime.trim(),
      ...
      fetch("/api/push", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          toAdmins: true, title: `Appointment request from ${name.trim()}`,
          body: anyTime ? "Any time works" : preferredTime.trim(),
          target: "adminAppointments", url: "/?open=adminAppointments",
        }),
      }).catch(() => {});
    } catch (e) { setErr("Couldn't send just now — have you run the appointment requests SQL?"); }
    finally { setBusy(false); }
  };
      ...
      <>
        ...
        <div style={{ background: T.card, borderRadius: 20, padding: 24, boxShadow: T.soft, marginTop: 20, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 14.5, color: T.sub, lineHeight: 1.55 }}>
            Juan's got your details and will {preferText ? "text" : "call"} you on {contact} to confirm your appointment time.
          </p>
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6, marginBottom: 4 }}>
        <CalendarCheck size={18} color={T.greenDk} />
        <h2 style={{ fontSize: 18, margin: 0 }}>Request Intake Appointment</h2>
      </div>
      <p style={{ fontSize: 13.5, color: T.sub, margin: "0 2px 16px", lineHeight: 1.5 }}>
        You can use this form to let Juan know the best times to book your initial intake appointment. Juan will call
        you on the number you provided to confirm your availability and lock your preferred appointment time in.
      </p>
      <div style={{ background: T.card, borderRadius: 20, padding: 18, boxShadow: T.soft }}>
        ...
        <label style={{ fontSize: 13, fontWeight: 700, display: "block", marginBottom: 6 }}>Contact number</label>
        <input value={contact} onChange={(e) => setContact(e.target.value)} placeholder="e.g. 04XX XXX XXX"
          type="tel" style={{ ...inputStyle, marginBottom: 16 }} />

        <label style={{ fontSize: 13, fontWeight: 700, display: "block", marginBottom: 8 }}>
          When would you prefer your appointment to be made?
        </label>
        <label style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, cursor: "pointer" }}>
          <input type="checkbox" checked={anyTime} onChange={(e) => setAnyTime(e.target.checked)} style={{ width: 18, height: 18 }} />
          <span style={{ fontSize: 14 }}>Any time</span>
        </label>
        {!anyTime && (
          <textarea value={preferredTime} onChange={(e) => setPreferredTime(e.target.value)} rows={3}
            placeholder="e.g. Weekday mornings, or Tuesday/Thursday after 3pm"
            style={{ ...inputStyle, resize: "none", minHeight: 80, marginBottom: 6 }} />
        )}

        <label style={{ display: "flex", alignItems: "center", gap: 8, margin: "18px 0 16px", cursor: "pointer" }}>
          <input type="checkbox" checked={preferText} onChange={(e) => setPreferText(e.target.checked)} style={{ width: 18, height: 18 }} />
          <span style={{ fontSize: 14 }}>Tick if you'd prefer Juan to text you rather than call</span>
        </label>

        {err && <p style={{ fontSize: 12.5, color: "#c94f4f", marginBottom: 10 }}>{err}</p>}
        <Btn onClick={submit} disabled={!canSubmit || busy}>{busy ? "Sending…" : "Submit request"}</Btn>
      </div>
    </>
  );
}
...
function ProgramInfo({ onBack, onMessageJuan, onRequestAppointment }) {
  return (
    <>
      <Brand right={<BackBtn onBack={onBack} label="Home" />} />

      <div style={{ background: T.card, borderRadius: 20, padding: "26px 20px", boxShadow: T.soft, marginTop: 8, textAlign: "center" }}>
        <div style={{ width: 56, height: 56, borderRadius: 16, background: T.green, color: "#fff", display: "grid",
          placeItems: "center", fontWeight: 800, fontSize: 18, boxShadow: T.soft, margin: "0 auto 14px" }}>RH</div>
      ...
      <ProgramInfoSection title="How it works">
        <ProgramInfoCard title="Duration">8 weeks</ProgramInfoCard>
      ...
      <ProgramInfoSection title="Download the intake pack">
        ...
        <div style={{ marginTop: 14 }}>
          <Btn onClick={onRequestAppointment}>Request Intake Appointment</Btn>
        </div>
      </ProgramInfoSection>
      ...
      <ProgramInfoSection title="Get in touch">
        <div style={{ background: "#eaf6ef", borderRadius: 16, padding: 16, marginBottom: 12 }}>
          <div style={{ fontSize: 13.5, lineHeight: 1.8 }}>
            <div>📍 7 Urana St, Villawood NSW 2163</div>
            <div>📞 0489 059 833 — call or text any time, 24/7</div>
          </div>
        </div>
        <p style={{ fontSize: 13, color: T.sub, margin: "0 0 14px", lineHeight: 1.5 }}>
          Want more detail, or ready to join? Reach out to Juan directly — he'll talk you through it and build the plan together with you.
        </p>
        <div style={{ display: "flex", gap: 10 }}>
