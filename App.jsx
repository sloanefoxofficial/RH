...
function primeAudio() {
  // Play a short silent clip to "warm"/unlock the shared audio element. Resolves
  // TRUE only if playback actually started (so callers know it really unlocked).
  if (typeof window === "undefined") return Promise.resolve(false);
  const a = getTtsAudio();
  if (!a) return Promise.resolve(false);
  return new Promise((resolve) => {
    try {
      // Detach speak()'s handlers so this priming can't flip the speaking state.
      const keep = { onplay: a.onplay, onended: a.onended, onerror: a.onerror };
      a.onplay = null; a.onended = null; a.onerror = null;
      const restore = () => { try { a.pause(); a.currentTime = 0; a.removeAttribute("src"); } catch {}
        a.onplay = keep.onplay; a.onended = keep.onended; a.onerror = keep.onerror; };
      a.src = __SILENT;
      const p = a.play();
      if (p && p.then) p.then(() => { restore(); resolve(true); }, () => { restore(); resolve(false); });
      else { restore(); resolve(true); }
    } catch { resolve(false); }
  });
}
...
function useVoice(voiceOn) {
  ...
  const speak = useCallback(async (text, char, onDone, force) => {
    ...
    if (char && char.voiceId) {
      // Speak the first short chunk as soon as it's ready; synthesise the rest
      // while it plays, so the guide starts talking after ~one sentence.
      const chunks = splitForTts(text);
      const prefetch = (i) => { if (i >= 0 && i < chunks.length) fetchTtsUrl(chunks[i], char.voiceId).catch(() => {}); };
      prefetch(0); prefetch(1);
      let idx = 0;
      const playNext = async () => {
        if (stale()) return;
        if (idx >= chunks.length) { setSpeaking(false); if (onDone) onDone(); return; }
        const chunk = chunks[idx++]; prefetch(idx);
        let url;
        try { url = await fetchTtsUrl(chunk, char.voiceId); }
        catch { if (!stale()) { if (idx === 1) browserSpeak(text, char, onDone); else playNext(); } return; }
        if (stale()) return;
        ...
        audio.onended = () => { if (stale()) return; playNext(); };
        audio.onerror = () => { if (!stale()) playNext(); };
        try { audio.src = url; audio.currentTime = 0; await audio.play(); }
        catch {
          try { await primeAudio(); if (stale()) return; audio.src = url; audio.currentTime = 0; await audio.play(); }
          catch { if (!stale()) { if (idx === 1) browserSpeak(text, char, onDone); else playNext(); } }
        }
      };
      playNext();
      return;
...
function HoldToTalk({ onText, onStart, size = 52 }) {
  ...
  const runSession = () => {
    ...
    let interimText = "";
    r.onresult = (e) => {
      // Append finalised segments; also remember the latest interim so the tail
      // isn't lost if the button is released before it finalises.
      let it = "";
      for (let i = e.resultIndex; i < e.results.length; i++) {
        const res = e.results[i]; if (!res || !res[0]) continue;
        if (res.isFinal) committedRef.current = (committedRef.current + " " + res[0].transcript.trim()).trim();
    ...
    r.onend = () => {
      ...
      setListening(false);
      const t = (committedRef.current + " " + interimText).trim();
      if (t) onText(t);
    };
...
function Chat({ char, profile, answers, history, setHistory, plan, progress, persona, memories, onConversation, voiceOn, setVoiceOn, onBack, onOpenTool, embedded, bargeIn }) {
  ...
  const hfListen = useCallback(() => {
    ...
    try {
      ...
      const arm = () => { clearTimeout(silence); silence = setTimeout(() => { try { r.stop(); } catch {} }, 1900); };
      r.onstart = () => arm();
      r.onresult = (e) => {
        interim = "";
        for (let i = e.resultIndex; i < e.results.length; i++) {
          const res = e.results[i]; if (!res || !res[0]) continue;
          if (res.isFinal) finalText = (finalText + " " + res[0].transcript.trim()).trim();
