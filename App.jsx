    ...
    <div style={{ minHeight: "100vh", color: T.ink,
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "0 16px 132px", position: "relative", zIndex: 1, zoom: textScale }}>
        {authEnabled && !authChecked ? (
          ...
          <RexIntro voiceOn={voiceOn} onReady={() => go("planChoice")} />
        ) : screen === "planChoice" ? (
  ...
  const speak = useCallback(async (text, char, onDone, force) => {
    ...
    if (char && char.voiceId) {
      try {
        ...
        audio.onended = () => { if (stale()) return; setSpeaking(false); if (onDone) onDone(); };
...
const REX_INTRO_LINES = [
  ...
  "You'll find Carlos inside your 8-week program tab — he's right there with your plan whenever you want to talk.",
  ...
  "If English isn't your first language, you can talk to your guides in whichever language you're most comfortable in.",
  "Just write to them in it, or simply ask them to use it, and they'll switch straight over.",
  "You can also save it in your profile under what the guides remember, so they'll know for next time.",
  "Next up, I'll ask you a few questions so the guides can shape an 8-week plan just for you.",
  "Here's the important bit: the more open and detailed your answers, the more personalised your plan will be.",
    ...
    <>
      ...
      <div style={{ paddingTop: 8, textAlign: "center" }}>
        ...
        <p style={{ fontSize: 12, color: T.sub, marginTop: 14, lineHeight: 1.5 }}>
          You can start a plan any time from the 8-week program page.
        </p>
      </div>
    </>
  );
...
function RexIntro({ voiceOn, onReady }) {
  ...
  useEffect(() => {
    ...
    const goNext = () => {
      ...
      pauseTimer.current = setTimeout(() => setIdx((k) => k + 1), 0);
    };
    ...
    <>
      ...
      <div style={{ paddingTop: 8, textAlign: "center" }}>
        ...
        <Bubble>
          ...
          so hang tight while I work through it. Once it's ready, you'll find me right inside your 8-week plan — come
...
function ProgramPage({ profile, plan, progress, saveProgress, journalCount, onOpenChat, onOpenJournal, onStartPlan, onBack }) {
  ...
  return (
    <div style={embedded ? {} : { minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: 8, paddingTop: 6 }}>
        ...
        <textarea ref={composerRef} value={input} onChange={(e) => setInput(e.target.value)} rows={4}
          onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }}
          placeholder={pendingImage ? "Say something about the photo (optional)…" : `Message ${char.name}…`}
          style={{ flex: 1, resize: "none", borderRadius: 16, border: `1px solid ${T.line}`, padding: "12px 14px",
            fontSize: 15, lineHeight: 1.45, minHeight: 96, maxHeight: 260, background: "#fff", color: T.ink,
