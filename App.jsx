...
export default function App() {
  ...
  const [journalPinSet, setJournalPinSet] = useState(false); // device-local privacy lock; PIN digest never leaves this device
  ...
  const setJournalPin = useCallback(async (pin) => {
    ...
    await sset(JOURNAL_PIN_STORAGE_KEY, { hash, createdAt: Date.now() });
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      <button onClick={() => setOpen((value) => !value)} aria-expanded={open}
        ...
        {open ? <ChevronLeft size={20} color={T.sub} style={{ transform: "rotate(-90deg)" }} /> : <ChevronRight size={20} color={T.sub} />}
      </button>
      {open && <div style={{ marginTop: 10 }}><NevernSpotlightContent /></div>}
    </div>
  );
}
...
function CarlosSpotlight() {
  ...
  const videos = [
    "https://vt.tiktok.com/ZSVW3U5dF/",
    "https://vt.tiktok.com/ZSVW3byM1/",
    "https://vt.tiktok.com/ZSVWTYxoU/",
    "https://vt.tiktok.com/ZSVW3sDPP/",
  ];
  return (
    <div style={{ background: T.card, borderRadius: 20, padding: 16, boxShadow: T.soft, marginBottom: 18 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
        <div style={{ width: 46, height: 46, borderRadius: "50%", background: "#e2eefb", display: "grid", placeItems: "center", flexShrink: 0 }}>
          <Shield size={22} color={T.blueDk} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {videos.map((url, index) => (
          <a key={url} href={url} target="_blank" rel="noopener noreferrer"
            style={{ display: "flex", alignItems: "center", gap: 12, background: "#f4f8fd", borderRadius: 14, padding: 12,
              textDecoration: "none", color: T.ink }}>
            <div style={{ width: 34, height: 34, borderRadius: 10, background: "#e2eefb", display: "grid", placeItems: "center", flexShrink: 0 }}>
              <Play size={15} color={T.blueDk} />
            </div>
            <span style={{ flex: 1, fontSize: 13.5, fontWeight: 600 }}>Carlos’s video {index + 1}</span>
            <ExternalLink size={15} color={T.sub} />
          </a>
        ))}
      </div>
    ...
    <>
      ...
      {groups.map((g) => (
        <div key={g.label} style={{ marginBottom: 18 }}>
          ...
          {g.label === "Calm down now" && <NevernSpotlight />}
        </div>
      ))}
      <Disclaimer />
    </>
...
function Settings({ textScale, reduceMotion, bargeIn, responseSpeed, speechLang, journalPinSet, onSetJournalPin, onClearJournalPin, installPromptAvailable, isStandalone, onPromptInstall, session, authEnabled, onSave, onBack, onOpenBugReport, onOpenFeedback }) {
    ...
    <>
      ...
      <div style={{ background: T.card, borderRadius: 18, padding: 16, boxShadow: T.soft, marginTop: 14 }}>
        ...
        {isStandalone ? (
          ...
          <>
            <Btn kind="outline" onClick={startInstall} style={{ width: "100%" }}>{installPromptAvailable ? "Add app to Home Screen" : "How to add to Home Screen"}</Btn>
            ...
            {showInstallSteps && (
              <div style={{ background: "#f6f5f8", borderRadius: 12, padding: 12, marginTop: 12, fontSize: 12.5, color: T.ink, lineHeight: 1.5 }}>
                {isIOS ? (
                  <>On iPhone or iPad, open this app in <strong>Safari</strong>, tap the <strong>Share</strong> button, scroll down, then choose <strong>Add to Home Screen</strong>. Tap <strong>Add</strong> to finish.</>
                ) : (
      ...
      <div style={{ background: T.card, borderRadius: 18, padding: 16, boxShadow: T.soft, marginTop: 14 }}>
        ...
        {!journalPinSet ? (
          <>
            <p style={{ fontSize: 12, color: T.sub, lineHeight: 1.45, margin: "12px 0" }}>The PIN is stored only on this device. It locks the Journal when you leave it or put the app in the background.</p>
          ...
          <>
            ...
            <p style={{ fontSize: 12, color: T.sub, lineHeight: 1.45, margin: "0 0 12px" }}>For privacy, the Journal locks whenever you leave it or put the app in the background. Your PIN cannot be recovered if forgotten.</p>
    ...
    <>
      ...
      <div style={{ background: T.card, borderRadius: 20, padding: 24, boxShadow: T.soft, marginTop: 20, textAlign: "center" }}>
        ...
        <p style={{ fontSize: 11.5, color: T.sub, lineHeight: 1.45, margin: "16px 0 0" }}>
          This is a privacy lock for the app on this device. For safety, the PIN cannot be recovered if it is forgotten.
        </p>
      </div>
    </>
  );
