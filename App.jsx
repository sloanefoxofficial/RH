...
function RexIntro({ voiceOn, onReady }) {
  ...
  useEffect(() => {
    ...
    if (voiceOn) { try { speak(line, CHARS.rex); } catch {} } // play audio, but don't depend on it to advance
    const readMs = Math.max(3400, line.length * 62); // enough time to hear or read the line
    timer.current = setTimeout(() => {
      if (idx >= REX_INTRO_LINES.length - 1) { setDone(true); return; }
      setThinking(true);
      thinkTimer.current = setTimeout(() => setIdx((k) => k + 1), 1100); // "Rex is thinking" beat
    }, readMs);
    return () => { clearTimeout(timer.current); clearTimeout(thinkTimer.current); };
  }, [idx, done, voiceOn, speak]);

  const skip = () => { clearTimeout(timer.current); clearTimeout(thinkTimer.current); stop(); setThinking(false); setDone(true); };
  useEffect(() => () => stop(), [stop]);
    ...
    <>
      ...
      <div style={{ paddingTop: 8, textAlign: "center" }}>
        ...
        <div className="rh-in" key={idx + (done ? "-done" : "")} style={{ background: T.card, borderRadius: 22,
          ...
          {thinking ? (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, color: T.sub, fontStyle: "italic" }}>
              <Sparkles size={16} color={T.green} /> Rex is thinking…
            </div>
          ) : (
    ...
    <>
      ...
      {plan && (
        <>
          ...
          <div style={{ background: T.card, borderRadius: 20, padding: 16, boxShadow: T.soft }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <button onClick={() => setWk(Math.max(1, wk - 1))} disabled={wk <= 1} style={navBtn(wk <= 1)}><ChevronLeft size={18} /></button>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 12, color: T.sub }}>Week {week?.n}</div>
                <div style={{ fontWeight: 700 }}>{week?.focus}</div>
              </div>
              <button onClick={() => setWk(Math.min(weeks.length, wk + 1))} disabled={wk >= weeks.length} style={navBtn(wk >= weeks.length)}><ChevronRight size={18} /></button>
            </div>
