...
- What the guides remember: to feel familiar, guides keep a few plain notes about them; sensitive things (like anything about feeling unsafe) are never kept; and they can view, edit, turn it off, or clear it under "What the guides remember" in their profile.
    ...
    <div style={{ minHeight: "100vh", color: T.ink,
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "0 16px 132px", position: "relative", zIndex: 1, zoom: textScale }}>
        {authEnabled && !authChecked ? (
          ...
          <GuidesPage onOpenChat={(slug) => go("chat", slug)} onBack={() => go("hub")} />
        ) : screen === "merch" ? (
          <MerchPage onBack={() => go("hub")} />
        ) : screen === "chat" ? (
          <Chat
          ...
          <Admin isAdmin={isAdmin} guidePrompts={guidePrompts} onSaveGuidePrompt={saveGuidePrompt} onBack={() => go("hub")} />
        ) : screen === "profile" ? (
          <Profile session={session} onReset={resetAll} onOpenMemory={() => go("memory")} onBack={() => go("hub")} />
        ) : screen === "notifications" ? (
          <Notifications session={session} onBack={() => go("hub")} />
        ) : screen === "coordinator" ? (
          <CoordinatorChat session={session} onBack={() => go("hub")} />
        ) : screen === "memory" ? (
          <MemoryManager memories={memories} memoryOn={memoryOn}
            onSave={(list, on) => saveMemories(list, on)} onBack={() => go("profile")} />
        ) : screen === "settings" ? (
          <Settings textScale={textScale} reduceMotion={reduceMotion} bargeIn={bargeIn}
            onSave={saveSettings} onBack={() => go("hub")} />
        ) : null}
      </div>
      <CrisisBar />
    </div>
...
function RexIntro({ voiceOn, onReady }) {
  ...
  useEffect(() => {
    ...
    const goNext = () => {
      ...
      pauseTimer.current = setTimeout(() => setIdx((k) => k + 1), 550); // natural beat after he finishes
    };
    if (voiceOn) {
    ...
    <>
      ...
      <Brand right={
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={shareApp} aria-label="Share the app" title="Share"
            style={{ background: "#fff", border: `1px solid ${T.line}`, borderRadius: 999, width: 38, height: 38,
              display: "grid", placeItems: "center", cursor: "pointer", color: T.sub, boxShadow: T.soft }}>
            <Share2 size={16} />
          </button>
      ...
      {/* Rex, live */}
      <SectionTitle>Rex — your welcomer</SectionTitle>
      <Chat char={CHARS.rex} profile={profile} answers={answers}
        history={rexHistory} setHistory={onSaveRexChat}
        memories={memories} onConversation={onConversation}
        voiceOn={voiceOn} setVoiceOn={setVoiceOn} onBack={() => {}} onOpenTool={() => {}} embedded bargeIn={bargeIn} persona={rexPersona} />
  ...
  return (
    <div style={embedded ? {} : { minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 8 }}>
        <p style={{ fontSize: 11, color: T.sub, textAlign: "center", margin: 0 }}>Hold the mic to talk · You never have to walk it alone.</p>
        <PrivacyLink style={{ fontSize: 11 }} />
      </div>
      {history.length > 0 && (
        <button onClick={() => setConfirmClear(true)} style={{ display: "block", margin: "8px auto 0", background: "none",
          border: "none", color: T.sub, fontSize: 12, cursor: "pointer", textDecoration: "underline" }}>
          Clear this chat
        </button>
      )}
      {confirmClear && (
