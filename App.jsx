...
function HoldToTalk({ onText, onStart, size = 52 }) {
  ...
  const start = () => {
    ...
    // A brief pause, plus the mic "wake" above, gives iOS time to actually
    // hand the mic back after any guide voice playback was just stopped —
    // starting recognition in the same tick can otherwise silently capture
    // nothing on iPhone.
    setTimeout(() => {
      if (!heldRef.current) return;
