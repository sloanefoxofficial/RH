  ...
  const send = async () => {
    ...
    try {
      ...
      fetch("/api/push", {
        ...
        body: JSON.stringify({ broadcast: true, title: title.trim(), body }),
      }).catch(() => {});
...
function Settings({ textScale, reduceMotion, bargeIn, responseSpeed, session, authEnabled, onSave, onBack }) {
  const [pushState, setPushState] = useState("checking"); // "checking" | "on" | "off" | "denied" | "unsupported"
  ...
  const togglePush = async () => {
    ...
    if (pushState === "on") { await unsubscribeFromPush(session.user.id); setPushState("off"); }
      ...
      setPushState(result === "granted" ? "on" : result === "denied" ? "denied" : "off");
    }
    setPushBusy(false);
  };
