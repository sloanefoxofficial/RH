...
function RexIntro({ voiceOn, onReady }) {
  ...
  useEffect(() => {
    ...
    const advance = () => {
      ...
      setThinking(true);
      timer.current = setTimeout(() => setIdx((k) => k + 1), 1100); // "Rex is thinking" beat
    };
    if (voiceOn) {
      speak(line, CHARS.rex, advance);
    } else {
      const readMs = Math.max(2600, line.length * 55);
      timer.current = setTimeout(advance, readMs);
    }
    return () => { if (timer.current) clearTimeout(timer.current); };
  }, [idx, done, voiceOn, speak]);

  const skip = () => { if (timer.current) clearTimeout(timer.current); stop(); setThinking(false); setDone(true); };
  useEffect(() => () => stop(), [stop]);
