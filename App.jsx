...
import {
  ...
  Shield, Eye, EyeOff, User, Megaphone,
} from "lucide-react";
...
const CHARS = {
  rex: {
    ...
    img: IMG.rex, tint: "#dff5e4", voice: { pitch: 1.05, rate: 1.03 }, voiceId: "en-AU-Neural2-C",
    system: `${SHARED}
You ARE Rex, the friendly face of The Resilience Hub — the first hello. You're warm, upbeat and reassuring. You help people feel safe, get their bearings, and take a small first step. When it fits, gently point them to the right guide: Juan for lived-experience mateship, Carlos for clinical tools, Mick for practical life and housing, Lila for family and relationships. You're the welcomer, not a specialist — for deeper support, warmly hand off to the right guide.`,
  ...
  juan: {
    ...
    img: IMG.juan, tint: "#f6e2a3", voice: { pitch: 0.9, rate: 1.02 }, voiceId: "en-AU-Neural2-B",
    system: `${SHARED}
You ARE Juan Carroso — the main voice. Speak first person, like a trusted mate: plain, honest, warm, sometimes cheeky, never talk down. You've walked through hard times — been homeless, worked tough jobs — and climbed out, so you get it even when someone mumbles or swears. You say things like "Mate, I've been there" and "One small step is still a step forward". You're still walking it too, just further down the road. For anything clinical, tap in Carlos. For housing/bills/government stuff, tap in Mick. For family and relationships, tap in Lila.`,
  ...
  carlos: {
    ...
    img: IMG.carlos, tint: "#b3d1f2", voice: { pitch: 1.0, rate: 0.98 }, voiceId: "en-AU-Neural2-D",
    system: `${SHARED}
You ARE Carlos Camacho, Registered Psychologist, author of "How To Be Happy For Adults". You work hand-in-hand with Juan and never contradict his lived experience — you ground it with simple, everyday-language tools (ACT, CBT, mindfulness). Wise, warm, precise, kind. Anything you suggest is a supportive idea the person can choose, never treatment or a diagnosis.`,
  ...
  mick: {
    ...
    img: IMG.mick, tint: "#aec6e2", voice: { pitch: 0.85, rate: 1.0 }, standby: true, voiceId: "en-AU-Wavenet-D",
    system: `${SHARED}
You are Mick — calm, practical support for real-life logistics: housing, bills, daily stability, and navigating government or community systems (Centrelink, tenancy, utilities). You're tapped in when Juan or Carlos needs a specialist hand; you never take over the main journey. Break scary admin into one small step at a time. For legal, financial or safety-critical matters, point to the right official service rather than giving definitive advice.`,
  ...
  lila: {
    ...
    img: IMG.lila, tint: "#f4d8c8", voice: { pitch: 1.12, rate: 1.0 }, standby: true, voiceId: "en-AU-Neural2-A",
    system: `${SHARED}
...
function useVoice(voiceOn) {
  ...
  const browserSpeak = useCallback((text, char) => {
  ...
  const speak = useCallback(async (text, char) => {
    ...
    // Prefer natural ElevenLabs voices when the guide has a voiceId; fall back
    // to the browser voice if the key isn't set, the call fails, or playback is blocked.
    if (char && char.voiceId) {
      try {
        ...
        if (myReq !== reqRef.current) return;      // a newer speak() superseded this
        if (res.ok) {
          ...
          audio.onended = () => { setSpeaking(false); try { URL.revokeObjectURL(url); } catch {} };
          audio.onerror = () => { setSpeaking(false); browserSpeak(text, char); };
          try { await audio.play(); return; }
          catch { browserSpeak(text, char); return; }
        }
      } catch { /* fall through to browser voice */ }
      if (myReq !== reqRef.current) return;
    }
    browserSpeak(text, char);
  }, [voiceOn, stop, browserSpeak]);

  return { speak, stop, speaking };
}

function HoldToTalk({ onText, size = 52 }) {
  ...
  const start = () => {
    ...
    setErr(null); finalRef.current = "";
    try {
      const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
      const r = new SR(); r.lang = "en-AU"; r.interimResults = true; r.continuous = true;
      let committed = false; // hand the transcript back exactly once per hold
      r.onresult = (e) => {
        // Rebuild the whole transcript every fire (never append), so repeated
        // events or a browser auto-restart can't stack duplicated text.
        let finalT = "", interimT = "";
        for (let i = 0; i < e.results.length; i++) {
          const seg = e.results[i][0] ? e.results[i][0].transcript : "";
          if (e.results[i].isFinal) finalT += seg + " "; else interimT += seg;
        }
        finalRef.current = (finalT + interimT).replace(/\s+/g, " ").trim();
      };
    ...
    } catch { setErr("Couldn't start the mic."); }
  };
  ...
  return (
    <>
      ...
      <p style={{ fontSize: 13, color: T.sub, margin: "0 2px 10px", lineHeight: 1.5 }}>
        Post a dated note about what's new — every signed-in member can read it under "App updates".
      </p>
      <AdminUpdates />
...
function Updates({ onBack }) {
    ...
    (async () => {
      ...
      try {
        const { data } = await supabase.from("app_updates")
          .select("id,update_date,title,body").order("update_date", { ascending: false });
        setRows(data || []);
      } catch { setRows([]); }
    })();
  }, []);
  return (
    <>
      ...
      <p style={{ fontSize: 13, color: T.sub, margin: "0 2px 12px", lineHeight: 1.5 }}>
        Updates and improvements to The Resilience Hub.
      </p>
      {rows === null && <div style={{ color: T.sub, fontSize: 14, padding: "6px 2px" }}>Loading…</div>}
      {rows && rows.length === 0 && <div style={{ color: T.sub, fontSize: 14, padding: "6px 2px" }}>No updates yet — check back soon.</div>}
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {(rows || []).map((u) => (
          <div key={u.id} style={{ background: T.card, borderRadius: 16, padding: 16, boxShadow: T.soft }}>
            <div style={{ fontSize: 12, color: "#7c5cc4", fontWeight: 700 }}>{fmtUpdDate(u.update_date)}</div>
...
function AdminUpdates() {
  ...
  const load = async () => {
    ...
    try {
      const { data } = await supabase.from("app_updates")
        .select("id,update_date,title").order("update_date", { ascending: false });
      setRows(data || []);
    } catch {}
  };
  useEffect(() => { load(); }, []);
  const add = async () => {
    ...
    if (!supabase) { setStatus("Connect Supabase to post."); return; }
    setStatus("Posting…");
    try {
      const { error } = await supabase.from("app_updates")
        .insert({ update_date: date || todayStr(), title: title.trim(), body });
      if (error) throw error;
      setTitle(""); setBody(""); setStatus("Posted."); load();
    } catch (e) { setStatus("Couldn't post — have you run the updates SQL?"); }
  };
  const del = async (id) => {
    if (!supabase) return;
    try { await supabase.from("app_updates").delete().eq("id", id); load(); } catch {}
  };
  return (
    <div style={{ background: T.card, borderRadius: 18, padding: 16, boxShadow: T.soft, margin: "0 0 4px" }}>
      ...
      <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title — e.g. Added breathing tool"
        style={{ ...inputStyle, marginBottom: 8 }} />
      <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={2}
        placeholder="A short description of what changed…" style={{ ...inputStyle, resize: "none", minHeight: 62 }} />
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 10 }}>
        <Btn onClick={add} style={{ width: 120, height: 44 }}>Post</Btn>
        <span style={{ fontSize: 12.5, color: status === "Posted." ? T.greenDk : T.sub }}>{status}</span>
      </div>
      {rows.length > 0 && (
        <div style={{ marginTop: 14, borderTop: `1px solid ${T.line}`, paddingTop: 8 }}>
          {rows.map((u) => (
            <div key={u.id} style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "8px 0" }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 11.5, color: T.sub }}>{fmtUpdDate(u.update_date)}</div>
...
/* ---------- hub / dashboard ---------- */
function Hub({ profile, plan, progress, saveProgress, journalCount, voiceOn, setVoiceOn, onOpenChat, onOpenJournal, onOpenToolkit, onOpenUpdates, onReset, isAdmin, authEnabled, onOpenAdmin, onOpenProfile, onSignOut }) {
  const { speak, stop, speaking } = useVoice(voiceOn);
  ...
  return (
    <>
      ...
      <SectionTitle>What's new</SectionTitle>
      <button onClick={onOpenUpdates} style={{ width: "100%", background: T.card, borderRadius: 20, padding: 16,
        boxShadow: T.soft, border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, textAlign: "left" }}>
        <div style={{ width: 44, height: 44, borderRadius: 14, background: "#eee7f6", display: "grid", placeItems: "center" }}>
          <Megaphone size={20} color="#7c5cc4" />
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 700 }}>App updates</div>
          <div style={{ fontSize: 13, color: T.sub }}>See what's been added and improved</div>
        </div>
        <ChevronRight size={20} color={T.sub} />
      </button>
...
function Chat({ char, profile, answers, history, setHistory, voiceOn, setVoiceOn, onBack, onOpenTool }) {
  ...
  const send = async (raw) => {
    ...
    try {
      ...
      let msgs = newHist.slice(-20).map((m) => ({ role: m.role, content: m.content }));
      ...
      const tagRe = /<tool>\s*(breathing|grounding|affirmations|calm)\s*<\/tool>/i;
      const found = reply.match(tagRe);
      const tool = found ? found[1].toLowerCase() : null;
      const clean = reply.replace(/<tool>\s*(breathing|grounding|affirmations|calm)\s*<\/tool>/gi, "").trim();
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: 8, paddingTop: 6 }}>
        <HoldToTalk onText={(t) => send(t)} size={48} />
        <textarea value={input} onChange={(e) => setInput(e.target.value)} rows={1}
          onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }}
