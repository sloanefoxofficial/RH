...
import {
  ...
  Shield, Eye, EyeOff, User, Megaphone, Youtube, ExternalLink, Radio, Paperclip, MessageCircle, Share2, Flame, HelpCircle, Plus, Settings as SettingsIcon, CalendarCheck, Users, ShoppingBag, Gamepad2,
} from "lucide-react";
    ...
    <div style={{ minHeight: "100vh", color: T.ink,
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "0 16px 132px", position: "relative", zIndex: 1, zoom: textScale }}>
        {authEnabled && !authChecked ? (
          ...
          <GamesPage gameScores={gameScores} onScore={saveGameScore} onBack={back} />
        ) : screen === "chat" ? (
          <Chat
...
const GAMES = [
  { id: "neojack", title: "Neo Jack: Sky Defender", blurb: "Juan's arcade shooter — dodge, blast, survive.",
    tag: "Arcade", file: "neojack.html", tint: "#1a1030", ic: "#8f7bd6" },
  { id: "sudoku", title: "Sudoku", blurb: "Classic number puzzle — easy, medium & hard.",
    tag: "Puzzle", file: "sudoku.html", tint: "#e7f0ea", ic: "#2c7d50" },
...
function GamesPage({ gameScores, onScore, onBack }) {
  const [open, setOpen] = useState(null);
  ...
  // Bridge: games (in the iframe) request/report scores via postMessage.
  useEffect(() => {
    function onMsg(e) {
      const d = e.data;
      ...
      const reply = (best) => {
        try { frameRef.current && frameRef.current.contentWindow &&
          frameRef.current.contentWindow.postMessage({ type: "rh-best", game: d.game, best: best ?? null }, "*"); } catch {}
      };
      if (d.type === "rh-get-best") {
        reply(scoresRef.current[d.game] ?? null);
  ...
  if (game) {
    return (
      <>
        ...
        <div style={{ borderRadius: 18, overflow: "hidden", boxShadow: T.soft, background: "#07070c", border: `1px solid ${T.line}` }}>
        ...
        <p style={{ fontSize: 11.5, color: T.sub, textAlign: "center", marginTop: 10, lineHeight: 1.5 }}>
          Tip: tap "Full screen" for a bigger view. Your best scores are saved just for you, on this account.
        </p>
        <Disclaimer />
      </>
    );
...
function Hub({ profile, plan, progress, saveProgress, journalCount, voiceOn, setVoiceOn, onOpenChat, onOpenProgram, onOpenGuides, onOpenMerch, onOpenGames, onOpenToolkit, onOpenNotifications, onOpenCoordinator, onOpenSettings, onReset, isAdmin, authEnabled, onOpenAdmin, onOpenProfile, onSignOut, session, rexHistory, onSaveRexChat, memories, onConversation, answers, bargeIn, rexPersona }) {
  const { speak, stop, speaking } = useVoice(voiceOn);
