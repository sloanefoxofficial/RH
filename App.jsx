...
import {
  ...
  Shield, Eye, EyeOff, User, Megaphone, Youtube, ExternalLink, Radio, Paperclip, MessageCircle, Share2, Flame, HelpCircle, Plus, Settings as SettingsIcon, CalendarCheck, Users,
} from "lucide-react";
...
function RexIntro({ voiceOn, onReady }) {
  ...
  const words = REX_INTRO_LINES[idx].split(" ");
  ...
  useEffect(() => {
    ...
    const w = line.split(" ");
    setShownWords(0);
    if (voiceOn) { try { speak(line, CHARS.rex); } catch {} } // play audio, don't depend on it to advance
    const readMs = Math.max(3400, line.length * 62);            // time to hear/read the whole line
    const perWord = Math.max(45, (readMs - 700) / w.length);    // reveal words across (most of) that time
    let shown = 0;
    reveal.current = setInterval(() => {
      shown += 1; setShownWords(shown);
      if (shown >= w.length) clearInterval(reveal.current);
    }, perWord);
    timer.current = setTimeout(() => {
      if (idx >= REX_INTRO_LINES.length - 1) { setDone(true); return; }
      setIdx((k) => k + 1); // natural pause, then next line
    }, readMs);
    return () => { clearInterval(reveal.current); clearTimeout(timer.current); };
  }, [idx, done, voiceOn, speak]);

  const skip = () => { clearInterval(reveal.current); clearTimeout(timer.current); stop(); setShownWords(999); setDone(true); };
  useEffect(() => () => stop(), [stop]);
      ...
      <div style={{ paddingTop: 8, textAlign: "center" }}>
        ...
        <div className="rh-in" key={idx + (done ? "-done" : "")} style={{ background: T.card, borderRadius: 22,
          ...
          <p style={{ fontSize: 15.5, lineHeight: 1.55, margin: 0 }}>
            {words.map((word, wi) => (
              <span key={wi} style={{ color: (done || wi < shownWords) ? T.ink : "transparent", transition: "color .15s" }}>{word}{" "}</span>
            ))}
          </p>
        </div>
