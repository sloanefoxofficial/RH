...
import {
  ...
  Shield, Eye, EyeOff, User, Megaphone, Youtube, ExternalLink, Radio, Paperclip, MessageCircle, Share2, Flame, HelpCircle, Plus, Search, Settings as SettingsIcon, CalendarCheck, Users, ShoppingBag, Gamepad2,
} from "lucide-react";
...
function ProgramPage({ profile, plan, progress, saveProgress, journalCount, chats, onOpenChat, onOpenJournal, onStartPlan, onBack }) {
  ...
  // Build a dated conversation log from every guide chat: one entry per guide per
  // day, with the day's first message used as the "topic".
  const chatLog = [];
  Object.entries(chats || {}).forEach(([slug, msgs]) => {
    const guide = (typeof CHARS !== "undefined") && CHARS[slug];
    if (!guide || !Array.isArray(msgs)) return;
    const byDay = {};
    msgs.forEach((m) => {
      if (!m || !m.ts) return;
      const key = new Date(m.ts).toDateString();
      if (!byDay[key]) byDay[key] = { ts: m.ts, guide: guide.name, tint: guide.tint, img: guide.img, slug, topic: "" };
      byDay[key].ts = Math.max(byDay[key].ts, m.ts);
      if (m.role === "user" && !byDay[key].topic && typeof m.content === "string" && m.content.trim() && m.content !== "(sent a photo)") {
        byDay[key].topic = m.content.trim();
      }
    });
    Object.values(byDay).forEach((e) => chatLog.push(e));
  });
  chatLog.sort((a, b) => b.ts - a.ts);
    ...
    <>
      ...
      {chatLog.length > 0 && (
        <>
          ...
          <p style={{ fontSize: 13, color: T.sub, margin: "0 2px 10px", lineHeight: 1.5 }}>
            A log of when you've talked with your guides and what about — tap any to pick it back up.
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 8 }}>
            {chatLog.slice(0, 12).map((e, i) => (
              <button key={i} onClick={() => onOpenChat(e.slug)} style={{ display: "flex", alignItems: "center", gap: 12,
                width: "100%", textAlign: "left", background: T.card, borderRadius: 16, padding: "12px 14px",
                boxShadow: T.soft, border: "none", cursor: "pointer", color: T.ink }}>
                <img src={e.img} alt={e.guide} style={{ width: 40, height: 40, borderRadius: 12, objectFit: "cover", background: e.tint, flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                    <span style={{ fontWeight: 700, fontSize: 14 }}>{e.guide}</span>
                    <span style={{ fontSize: 12, color: T.sub, whiteSpace: "nowrap" }}>
                      {new Date(e.ts).toLocaleDateString(undefined, { day: "numeric", month: "short" })}</span>
                  </div>
                  <div style={{ fontSize: 13, color: T.sub, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {e.topic || "Chatted"}
                  </div>
...
/* ---------- chat ---------- */
function Chat({ char, profile, answers, history, setHistory, plan, progress, saveProgress, persona, memories, onConversation, voiceOn, setVoiceOn, onBack, onOpenTool, embedded, bargeIn }) {
  ...
  return (
    <div style={embedded ? {} : { minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      {searchOpen && (
        <div style={{ display: "flex", gap: 8, alignItems: "center", padding: "0 2px 8px" }}>
