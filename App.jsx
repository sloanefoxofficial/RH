...
async function subscribeToPush(userId) {
  ...
  try {
    ...
    if (error) return "error"; // e.g. the table doesn't exist yet — toggle looked "on" before but never actually saved
    return "granted";
  } catch { return "error"; }
}
    ...
    try {
      ...
      try {
        const r = await fetch("/api/push", {
          ...
          body: JSON.stringify({ broadcast: true, title: t, body }),
        });
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      try {
        const r = await fetch("/api/push", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId: open, title: "Juan replied", body: text, url: "/" }),
        });
        const d = await r.json().catch(() => null);
...
function Settings({ textScale, reduceMotion, bargeIn, responseSpeed, session, authEnabled, onSave, onBack }) {
  ...
  const togglePush = async () => {
    ...
    if (pushState === "on") { await unsubscribeFromPush(session.user.id); setPushState("off"); }
      ...
      setPushState(result === "granted" ? "on" : result);
    }
    setPushBusy(false);
  };
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12.5, color: T.sub, lineHeight: 1.4 }}>
              {pushState === "unsupported" ? "Not available on this browser."
                ...
                : pushState === "error" ? "Something went wrong saving this — try again in a moment, or let Lisa know if it keeps happening."
          ...
          {(pushState === "on" || pushState === "off") && (
            <button onClick={togglePush} disabled={pushBusy} aria-label="Toggle push notifications"
              style={{ width: 52, height: 30, borderRadius: 999, border: "none", cursor: pushBusy ? "default" : "pointer", flexShrink: 0,
                background: pushState === "on" ? T.green : "#cfc6da", position: "relative", transition: "background .2s", opacity: pushBusy ? 0.6 : 1 }}>
...
function Hub({ profile, plan, progress, saveProgress, journalCount, voiceOn, setVoiceOn, onOpenChat, onOpenProgram, onOpenGuides, onOpenMerch, onOpenGames, onOpenToolkit, onOpenNotifications, onOpenCoordinator, onOpenSettings, onOpenMensGroup, onReset, isAdmin, authEnabled, guestMode, onExitGuest, onOpenAdmin, onOpenProfile, onSignOut, session, rexHistory, onSaveRexChat, memories, onConversation, answers, bargeIn, rexPersona }) {
  const { speak, stop, speaking } = useVoice(voiceOn);
