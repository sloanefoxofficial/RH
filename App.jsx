      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "0 16px 132px", position: "relative" }}>
        {authEnabled && !authChecked ? (
          ...
          <Chat
            ...
            setHistory={(h) => saveChats({ ...chats, [activeChar]: h })}
            voiceOn={voiceOn} setVoiceOn={setVoiceOn}
            onBack={() => go("hub")}
            onOpenTool={openTool}
          />
    ...
    (async () => {
      ...
      try {
        ...
        if (data) setP({
          ...
          bio: data.bio || "", contact_private: data.contact_private || "", avatar: data.avatar || "",
        });
      } catch {}
    })();
  }, [session]);
  ...
  const save = async () => {
    ...
    try {
      const { error } = await supabase.from("profiles").upsert({
        ...
        contact_private: p.contact_private, avatar: p.avatar,
        updated_at: new Date().toISOString(),
      });
      if (error) throw error;
      setStatus("Saved.");
    } catch (e) { setStatus(e.message || "Couldn't save — is the profiles table set up?"); }
  };
      ...
      <div style={{ paddingTop: 8 }}>
        ...
        <div className="rh-in" style={{ background: T.card, borderRadius: 22, padding: 18, boxShadow: T.soft, margin: "18px 0" }}>
          ...
          <p style={{ margin: "0 0 14px", fontSize: 14.5, lineHeight: 1.5 }}>
            ...
            secure AI service so they can reply. Your journal, plan, and progress are saved only on this device.
          </p>
...
/* ---------- hub / dashboard ---------- */
function Hub({ profile, plan, progress, saveProgress, journalCount, voiceOn, setVoiceOn, onOpenChat, onOpenJournal, onOpenToolkit, onReset, isAdmin, authEnabled, onOpenAdmin, onOpenProfile, onSignOut }) {
  const { speak, stop, speaking } = useVoice(voiceOn);
