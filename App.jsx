...
import {
  ...
  Shield, Eye, EyeOff, User, Megaphone, Youtube, ExternalLink, Radio, Paperclip, MessageCircle, Share2, Flame, HelpCircle, Plus, Settings as SettingsIcon, CalendarCheck, Users, ShoppingBag,
} from "lucide-react";
...
const T = {
  bgTop: "#ece7f4", bgMid: "#eef0f7", bgBot: "#fdefe7",
  card: "#ffffff", ink: "#28262f", sub: "#726e7c",
  line: "#e9e3f0", green: "#37a065", greenDk: "#2c7d50",
  teal: "#2f9e93", tealDk: "#227d74",
  blue: "#3f6faf", blueDk: "#2f5a8c",
  soft: "0 6px 20px rgba(70,55,105,0.07), 0 2px 6px rgba(70,55,105,0.05)",
  lift: "0 20px 50px rgba(70,55,105,0.17), 0 6px 16px rgba(70,55,105,0.10)",
};
...
- Getting around: the home screen has a welcome, a card to chat with you (Rex), and cards for the 8-week program, the guides, the toolkit, the merch store, Message Juan, and notifications. Each card opens its own screen.
...
export default function App() {
  ...
  const saveSettings = useCallback(({ textScale: ts, reduceMotion: rm, bargeIn: bi }) => {
  ...
  const resetAll = async () => {
    for (const k of ["rh_profile", "rh_answers", "rh_plan", "rh_progress", "rh_journal", "rh_chats", "rh_memories", "rh_memory_on"]) {
    ...
    setJournal([]); setChats({}); setMemories([]); memoriesRef.current = [];
    setScreen("welcome");
  };
    ...
    <div style={{ minHeight: "100vh", color: T.ink,
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "0 16px 132px", position: "relative", zIndex: 1, zoom: textScale }}>
        {authEnabled && !authChecked ? (
          ...
          <RexIntro voiceOn={voiceOn} onReady={() => go("onboarding")} />
        ) : screen === "onboarding" ? (
          <Onboarding
          ...
          <Journal
            ...
            voiceOn={voiceOn} onBack={back}
          />
        ) : screen === "toolkit" ? (
          <Toolkit voiceOn={voiceOn} initial={toolkitInitial} onBack={back} />
        ) : screen === "admin" ? (
          ...
          <Settings textScale={textScale} reduceMotion={reduceMotion} bargeIn={bargeIn}
            onSave={saveSettings} onBack={back} />
        ) : null}
      </div>
      <CrisisBar />
    </div>
  ...
  return (
    <style>{`
      ...
      *{box-sizing:border-box}
      ...
      /* very soft dotted texture — adds life without any harsh flashes */
      body::before{content:"";position:fixed;inset:0;pointer-events:none;z-index:0;opacity:0.5;
        background-image:radial-gradient(rgba(90,70,120,0.05) 1px, transparent 1px);background-size:22px 22px}
...
function Brand({ right }) {
  return (
    <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "18px 2px 8px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        <div style={{ width: 38, height: 38, borderRadius: 12, background: T.green, color: "#fff",
          display: "grid", placeItems: "center", fontWeight: 800, boxShadow: T.soft, flexShrink: 0 }}>RH</div>
        <div style={{ lineHeight: 1.1 }}>
          <div style={{ fontWeight: 700, fontSize: 15, whiteSpace: "nowrap" }}>The Resilience Hub</div>
          <div style={{ fontSize: 11, color: T.sub, whiteSpace: "nowrap" }}>You never have to walk it alone</div>
        </div>
      </div>
      {right && (
...
const __SILENT = "data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YQAAAAA=";
...
function unlockAudio() {
  ...
  __audioUnlocked = true; // mark immediately so this can only ever run once
  const a = getTtsAudio();
  if (!a) return;
  try {
    // Play a silent clip WITHOUT any of speak()'s handlers attached, so this
    // priming can never fire onplay/onended (which would flip the speaking state).
    const keep = { onplay: a.onplay, onended: a.onended, onerror: a.onerror };
    a.onplay = null; a.onended = null; a.onerror = null;
    a.src = __SILENT;
    const done = () => { try { a.pause(); a.currentTime = 0; a.removeAttribute("src"); } catch {}
...
if (typeof window !== "undefined") {
  const h = () => {
    unlockAudio();
    // Only needed once — remove the listeners so no later tap can re-trigger it.
    ["pointerdown", "touchend", "click", "keydown"].forEach((ev) => window.removeEventListener(ev, h));
  };
  ["pointerdown", "touchend", "click", "keydown"].forEach((ev) => window.addEventListener(ev, h, { passive: true }));
}
...
function useVoice(voiceOn) {
  ...
  return { speak, stop, speaking, paused, pauseResume };
}
...
function HoldToTalk({ onText, onStart, size = 52 }) {
  ...
  const runSession = () => {
    ...
    r.lang = "en-AU"; r.interimResults = false; r.continuous = true;
    r.onresult = (e) => {
      // Append only newly-finalised segments (via resultIndex) so nothing is
      // ever counted twice. continuous:true keeps ONE session open while the
      // person keeps talking, so the mic doesn't restart (and re-chime) every
      // few seconds — it only restarts after a genuine pause.
...
function SelfHelpTool({ onBack }) {
  ...
  return (
    <>
      ...
      <p style={{ fontSize: 13.5, color: T.sub, margin: "0 2px 12px", lineHeight: 1.5 }}>
        A few channels worth a look, or search below for talks on anything you're working on.
      </p>

      {[
        { group: "Mindset & personal growth", items: [
          { name: "Tony Robbins", url: "https://youtube.com/@TonyRobbinsLive", desc: "World-renowned — resilience, overcoming fear, life direction, breaking old patterns" },
          { name: "Dr Julie Smith", url: "https://youtube.com/@drjulie", desc: "Clinical psychologist — clear, practical mental health tips, anxiety, self-doubt" },
          { name: "The School of Life", url: "https://youtube.com/@TheSchoolOfLife", desc: "Deep, gentle wisdom — self-understanding, relationships, calm, balance" },
          { name: "Tara Brach", url: "https://youtube.com/@TaraBrach", desc: "Mindfulness, self-compassion, healing trauma & stress" },
        ]},
        { group: "AI learning & practical skills", items: [
          { name: "Danny Why", url: "https://youtube.com/@danny_why", desc: "Simple, real-world AI tutorials — how to use tools, create content, save time" },
          { name: "Matt Wolfe", url: "https://youtube.com/@mreflow", desc: "No-nonsense AI guides — ChatGPT, Claude, tools for everyday people" },
        ]},
        { group: "Science & wellbeing", items: [
          { name: "Huberman Lab", url: "https://youtube.com/@hubermanlab", desc: "Neuroscience for everyday life — focus, sleep, energy, mental strength" },
        ]},
      ].map((sec) => (
        <div key={sec.group} style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 11.5, fontWeight: 700, color: T.sub, textTransform: "uppercase", letterSpacing: 0.5, margin: "0 2px 8px" }}>{sec.group}</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {sec.items.map((c) => (
              <a key={c.name} href={c.url} target="_blank" rel="noopener noreferrer"
                style={{ display: "flex", alignItems: "center", gap: 12, background: T.card, borderRadius: 16, padding: 13,
                  boxShadow: T.soft, textDecoration: "none", color: T.ink }}>
                <div style={{ width: 38, height: 38, borderRadius: 11, background: "#fde7cf", display: "grid", placeItems: "center", flexShrink: 0 }}>
                  <Flame size={18} color="#d0904e" />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: 14.5 }}>{c.name}</div>
                  <div style={{ fontSize: 12.5, color: T.sub, lineHeight: 1.4, marginTop: 1 }}>{c.desc}</div>
                </div>
                <ExternalLink size={15} color={T.sub} />
              </a>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 10 }}>
          {searches.map((sTxt, i) => (
            <a key={i} href={ytUrl(sTxt)} target="_blank" rel="noopener noreferrer"
              style={{ display: "flex", alignItems: "center", gap: 12, background: T.card, borderRadius: 16, padding: 14,
                boxShadow: T.soft, textDecoration: "none", color: T.ink }}>
              <div style={{ width: 40, height: 40, borderRadius: 12, background: "#fde7cf", display: "grid", placeItems: "center", flexShrink: 0 }}>
                <Flame size={20} color="#d0904e" />
              </div>
              <span style={{ flex: 1, fontSize: 14.5, fontWeight: 600, lineHeight: 1.35 }}>{sTxt}</span>
              <ExternalLink size={16} color={T.sub} />
            </a>
...
function Admin({ isAdmin, guidePrompts, onSaveGuidePrompt, onBack }) {
  ...
  return (
    <>
      ...
      <AdminSafetyPanel />
      <AdminWelcomeEditor />
      <GuidePersonalityEditor guidePrompts={guidePrompts} onSave={onSaveGuidePrompt} />

      <SectionTitle>Member messages</SectionTitle>
      <p style={{ fontSize: 13, color: T.sub, margin: "0 2px 10px", lineHeight: 1.5 }}>
        Private one-to-one messages from members to Juan. Only you see these.
      </p>
      <AdminInbox />
...
function AdminInbox() {
  ...
  const sendReply = async () => {
    ...
    try { await supabase.from("coordinator_messages").insert({ user_id: open, sender: "coordinator", body: text }); load(); }
...
function MemoryManager({ memories, memoryOn, onSave, onBack }) {
  ...
  return (
    <>
      <Brand right={<BackBtn onBack={onBack} label="Profile" />} />
      <SectionTitle>What the guides remember</SectionTitle>
      <p style={{ fontSize: 13.5, color: T.sub, margin: "0 2px 14px", lineHeight: 1.5 }}>
...
function Settings({ textScale, reduceMotion, bargeIn, onSave, onBack }) {
  const sizes = [
...
function ChatHelp() {
  ...
  const rows = [
    ...
    { Icon: Mic, title: "Talk instead of typing", body: "Hold the mic button down, say what you want, then let go — it turns your voice into a message. Works best in Chrome on Android or a computer." },
...
const REX_INTRO_LINES = [
  ...
  "This is a calm, private space to help you through whatever you're carrying — some support, some tools, and a few guides in your corner. You never have to walk it alone.",
  "It's for anyone doing it tough and wanting a hand. No judgement, ever — whatever you're going through, you belong here.",
  "Here's how it works: you can chat any time with our guides. Juan's your mate for anything at all, Carlos has calming tools for stress and low moments, Mick's great with practical life and housing, and Lila's there for family and relationships.",
  "Quick honest note: all of us guides are AI, not real people. Carlos here is an AI inspired by a real psychologist, Carlos Camacho — he offers supportive tools, not therapy or diagnosis. And if you'd ever like to do the in-person 8-week program with the real team, just tap Message Juan any time and the real Juan will get back to you.",
  "There's also a toolkit for calming down, a journal to get things out of your head, and an 8-week program to work through at your own pace. And I'm always here on the home screen if you need pointing in the right direction.",
  "One handy thing: tap the little person icon up the top of the home screen to open your profile. That's where you set your name and a bit about yourself, update your details or password, and see, change, or switch off what the guides remember about you — you're always in control of that.",
  "And right next to it there's a settings icon — tap that if you'd like to make the text bigger or smaller, anywhere in the app. Set it up however's easiest on your eyes.",
  "Speaking of that program — next up, I'll ask you a few questions so the guides can shape an 8-week plan just for you.",
  "Here's the important bit: the more open and detailed you are with your answers, the more personalised your plan will be. Take your time — there are no wrong answers.",
  "Whenever you're ready, tap the button below and we'll make a start.",
];

function RexIntro({ voiceOn, onReady }) {
  ...
  useEffect(() => {
    ...
    const goNext = () => {
      ...
      if (idx >= REX_INTRO_LINES.length - 1) { setDone(true); return; }
      pauseTimer.current = setTimeout(() => setIdx((k) => k + 1), 275); // short natural beat after he finishes
    };
    if (voiceOn) {
      ...
      timer.current = setTimeout(goNext, Math.max(3200, line.length * 60)); // silent: reading pace
    }
    return () => { clearTimeout(timer.current); clearTimeout(pauseTimer.current); };
  }, [idx, done, voiceOn, speak]);

  const skip = () => { clearTimeout(timer.current); clearTimeout(pauseTimer.current); stop(); setDone(true); };
  useEffect(() => () => stop(), [stop]);
  ...
  return (
    <>
      ...
      <div style={{ paddingTop: 8, textAlign: "center" }}>
        ...
        <div className="rh-in" key={idx + (done ? "-done" : "")} style={{ background: T.card, borderRadius: 22,
          padding: "18px 18px", boxShadow: T.soft, margin: "16px auto 0", maxWidth: 420, minHeight: 96,
...
function VoiceToggle({ on, set }) {
  return (
    <button onClick={() => set(!on)} aria-label={on ? "Voice on" : "Voice off"}
      style={{ background: "#fff", border: `1px solid ${T.line}`, borderRadius: 999, width: 34, height: 34,
...
function Onboarding({ profile, saveProfile, answers, saveAnswers, savePlan, voiceOn, onBackToIntro, onDone }) {
  const [i, setI] = useState(0);
...
{"summary":"2-3 warm sentences to them referencing their real situation and goal","weeks":[{"n":1,"focus":"short focus title","days":[{"d":1,"tasks":["task","task"]}, ... 7 days]}, ... all 8 weeks]}`;
  ...
  return (
    <>
      ...
      <Bubble>{q.q}</Bubble>
      {i === 0 && onBackToIntro && (
        <button onClick={() => { stop(); onBackToIntro(); }} style={{ display: "flex", alignItems: "center", gap: 6,
          margin: "0 auto 4px", background: "none", border: "none", color: T.green, fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
...
function fallbackPlan(name) {
  ...
  const weeks = focuses.map((focus, wi) => ({
    ...
    days: Array.from({ length: 7 }, (_, di) => ({
      d: di + 1,
      tasks: (di === 0 && [2, 4, 6].includes(wi))
        ? ["Plan review — type 'Plan Review' to Carlos to check in on how your plan's going", ...dayTasks[di]]
        : dayTasks[di],
    })),
  }));
  return { summary: `Here's an 8-week plan to get us started, ${name || "friend"} — a couple of small things each day, at your pace. Nothing's set in stone; we adjust as we go, and the guides are here whenever you want to talk.`, weeks };
}

/* ---------- hub / dashboard ---------- */
/* ---------- merch store ---------- */
const MERCH_URL = "https://sloanefox.bigcartel.com/";
const MERCH_ITEMS = [
  { name: "400GSM Elite Heavyweight Hoodie", price: "$139.95", opts: "4 options", img: "/merch/hoodie.png", url: "https://sloanefox.bigcartel.com/product/sloane-fox-400gsm-elite-heavyweight-hoodie" },
  { name: "Heavyweight Track Pants", price: "$109.95", opts: "4 options", img: "/merch/pants.png", url: "https://sloanefox.bigcartel.com/product/sloane-fox-heavyweight-track-pants" },
  { name: "Vixen Signature Tee", price: "$79.95", opts: "4 options", img: "/merch/vixen.png", url: "https://sloanefox.bigcartel.com/product/sloane-fox-space-age-galaxy-tee-hoodie" },
  { name: "280GSM Heavyweight Signature Tee", price: "$79.95", opts: "4 options", img: "/merch/signature.png", url: "https://sloanefox.bigcartel.com/product/sloane-fox-280gsm-heavyweight-tee" },
  { name: "Vertical Logo Tee", price: "$79.95", opts: "4 options", img: "/merch/vertical.png", url: "https://sloanefox.bigcartel.com/product/sloane-fox-vertical-logo-tee" },
];
  ...
  return (
    <>
          ...
          <a key={it.name} href={it.url} target="_blank" rel="noopener noreferrer"
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px" }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 13.5, color: T.green, fontWeight: 700 }}>{it.price} <span style={{ color: T.sub, fontWeight: 500 }}>· {it.opts}</span></div>
              </div>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        <a href="https://sloanefox.bigcartel.com/category/apparel" target="_blank" rel="noopener noreferrer"
        ...
        <a href="https://sloanefox.bigcartel.com/policies/shipping" target="_blank" rel="noopener noreferrer"
          style={{ flex: 1, textAlign: "center", background: T.card, borderRadius: 12, padding: "11px", fontSize: 13,
            fontWeight: 600, color: T.ink, textDecoration: "none", boxShadow: T.soft }}>Shipping</a>
        <a href="https://sloanefox.bigcartel.com/contact" target="_blank" rel="noopener noreferrer"
          style={{ flex: 1, textAlign: "center", background: T.card, borderRadius: 12, padding: "11px", fontSize: 13,
      ...
      <p style={{ fontSize: 11.5, color: T.sub, textAlign: "center", marginTop: 16, lineHeight: 1.5 }}>
        Shopping and checkout happen securely on our Big Cartel store, which opens in a new tab.
      </p>
    </>
  );
}
...
function ProgramPage({ profile, plan, progress, saveProgress, journalCount, onOpenChat, onOpenJournal, onBack }) {
  ...
  return (
    <>
      <Brand right={<BackBtn onBack={onBack} label="Hub" />} />
      <SectionTitle>Your 8-week program</SectionTitle>
              ...
              <>
                {(showAllDays ? week.days : week.days.slice(0, 3)).map((day) => (
                  <div key={day.d} style={{ borderTop: `1px solid ${T.line}`, paddingTop: 8, marginTop: 8 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: T.sub, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 }}>Day {day.d}</div>
      ...
      <SectionTitle>Your support guides</SectionTitle>
      <GuideRow char={CHARS.juan} onClick={() => onOpenChat("juan")} big />
      <GuideRow char={CHARS.carlos} onClick={() => onOpenChat("carlos")} big />
      <div style={{ background: "#eef4fb", border: `1px solid #d3e3f5`, borderRadius: 14, padding: 13, marginTop: 10 }}>
        <p style={{ fontSize: 12.5, color: T.sub, lineHeight: 1.55, margin: 0 }}>
          Carlos is an AI guide inspired by psychologist Carlos Camacho — he offers supportive tools, not therapy or
          diagnosis. To ask about the in-person 8-week program with the real team, message Juan.
        </p>
      </div>
      <Disclaimer />
    </>
...
function GuidesPage({ onOpenChat, onBack }) {
  ...
  return (
    <>
      ...
      <p style={{ fontSize: 13.5, color: T.sub, margin: "0 2px 16px", lineHeight: 1.5 }}>
        Specialists for specific parts of life. Tap to chat any time — and if you're not sure who to ask, Rex or Juan can point you the right way.
      </p>
      {specialists.map(({ char, forWhat, tip }) => (
...
function Hub({ profile, plan, progress, saveProgress, journalCount, voiceOn, setVoiceOn, onOpenChat, onOpenProgram, onOpenGuides, onOpenMerch, onOpenToolkit, onOpenNotifications, onOpenCoordinator, onOpenSettings, onReset, isAdmin, authEnabled, onOpenAdmin, onOpenProfile, onSignOut, session, rexHistory, onSaveRexChat, memories, onConversation, answers, bargeIn, rexPersona }) {
  const { speak, stop, speaking } = useVoice(voiceOn);
  ...
  const card = (onClick, tint, ic, Icon, title, sub, badge) => (
    <button onClick={onClick} style={{ width: "100%", background: T.card, borderRadius: 20, padding: 16,
      boxShadow: T.soft, border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, textAlign: "left" }}>
      <div style={{ width: 46, height: 46, borderRadius: 14, background: tint, display: "grid", placeItems: "center", position: "relative", flexShrink: 0 }}>
  ...
  return (
    <>
      ...
      <Brand right={<>
        ...
        {authEnabled && (
          <button onClick={onSignOut} aria-label="Sign out" title="Sign out" style={hubIconBtn(T.sub)}>
            <LogOut size={16} />
          </button>
        )}
      </>} />
      ...
      {/* welcome band */}
      <div style={{ background: T.card, borderRadius: 20, padding: "18px 18px", boxShadow: T.soft, marginTop: 6 }}>
        <div style={{ fontSize: 19, fontWeight: 800, marginBottom: 4 }}>Welcome to The Resilience Hub{nm ? `, ${nm}` : ""}</div>
        <div style={{ fontSize: 13.5, color: T.green, fontWeight: 600, marginBottom: 10 }}>You never have to walk it alone.</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {card(onOpenProgram, "#e7eefb", "#3f6faf", CalendarCheck, "Your 8-week program", "Plan, progress, journal & your support guides")}
        {card(onOpenGuides, "#f4e3d9", "#c9803f", Users, "Your guides", "Mick & Lila — specialists for specific things")}
        ...
        {card(onOpenNotifications, "#eee7f6", "#7c5cc4", Megaphone, "Notifications", unreadCount > 0 ? `${unreadCount} new message${unreadCount === 1 ? "" : "s"}` : "Messages from the team", unreadCount)}
      </div>

      <SectionTitle>Community</SectionTitle>
      <p style={{ fontSize: 13, color: T.sub, margin: "0 2px 10px", lineHeight: 1.5 }}>
        Connect with everyone on Facebook — jump straight to the group or the page any time.
      </p>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {[
...
const hubIconBtn = (color) => ({ background: "#fff", border: `1px solid ${T.line}`, borderRadius: 999, width: 34, height: 34,
  display: "grid", placeItems: "center", cursor: "pointer", color, boxShadow: T.soft, flexShrink: 0 });
...
/* ---------- chat ---------- */
function Chat({ char, profile, answers, history, setHistory, plan, progress, persona, memories, onConversation, voiceOn, setVoiceOn, onBack, onOpenTool, embedded, bargeIn }) {
  ...
  const startBargeWatch = useCallback(() => {
    ...
    if (!SR) return;
    try {
      const r = new SR(); r.lang = "en-AU"; r.interimResults = true; r.continuous = true;
      let triggered = false;
      r.onresult = (e) => {
        if (triggered) return;
        let seg = "";
  ...
  const send = async (raw) => {
    ...
    try {
      ...
      // Plan review: when the person types "Plan Review" to Carlos, hand him their
      // current plan and progress and have him walk them through it collaboratively.
      if (char.slug === "carlos" && /\bplan\s*review\b/i.test(text) && plan && Array.isArray(plan.weeks)) {
        ...
        system += `\n\n[PLAN REVIEW REQUESTED] The person wants to review their 8-week plan with you now. Their goal was: ${answers?.goal || "(not specified)"}. Progress so far — ${summary}. Gently walk them through it: celebrate what they've done, ask what's been working and what's felt hard or unrealistic, and talk through any changes they'd like (easier or harder, a different focus, more or less each day). Keep it warm and collaborative — it's their plan. If they settle on changes, describe the adjustments clearly so they know what to tweak; you don't rewrite it automatically.`;
      }
  ...
  return (
    <div style={embedded ? {} : { minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        ...
        {visibleHistory.map((m, idx) => {
          ...
          return (
            <div key={idx} style={{ display: "flex", flexDirection: "column", alignItems: m.role === "user" ? "flex-end" : "flex-start" }}>
              ...
              <div onClick={m.role === "assistant" ? () => { if (!pauseResume()) speak(m.content, char, undefined, true); } : undefined}
              ...
              {m.role === "assistant" && (
                <button onClick={() => speak(m.content, char, undefined, true)} aria-label="Repeat this"
      ...
      <p style={{ fontSize: 10.5, color: T.sub, textAlign: "center", marginTop: 4 }}>Photos only — our guides cannot view video files.</p>
      {speaking && (
      ...
      <div style={{ borderTop: `1px solid ${T.line}`, marginTop: 12, paddingTop: 10,
        display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", gap: "4px 10px" }}>
        <span style={{ fontSize: 11, color: T.sub }}>Hold the mic to talk</span>
        {history.length > 0 && (
          <>
      ...
      <p style={{ fontSize: 11, color: T.sub, textAlign: "center", margin: "6px 0 0", fontStyle: "italic" }}>
        You never have to walk it alone.
      </p>
      {confirmClear && (
        <div style={{ position: "fixed", inset: 0, zIndex: 120, background: "rgba(44,42,51,0.45)",
