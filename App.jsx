...
export default function App() {
  ...
  const saveSettings = useCallback(({ textScale: ts, reduceMotion: rm }) => {
    ...
    <div style={{ minHeight: "100vh", color: T.ink,
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "0 16px 132px", position: "relative", zIndex: 1, zoom: textScale }}>
        {authEnabled && !authChecked ? (
          ...
          <Settings textScale={textScale} reduceMotion={reduceMotion}
...
function Settings({ textScale, reduceMotion, onSave, onBack }) {
  const sizes = [
...
function fallbackPlan(name) {
  ...
  const weeks = focuses.map((focus, wi) => ({
    n: wi + 1, focus,
    days: Array.from({ length: 7 }, (_, di) => ({ d: di + 1, tasks: dayTasks[di] })),
  }));
  ...
  return (
    <>
      ...
      <Chat char={CHARS.rex} profile={profile} answers={answers}
        ...
        voiceOn={voiceOn} setVoiceOn={setVoiceOn} onBack={() => {}} onOpenTool={() => {}} embedded />
...
/* ---------- chat ---------- */
function Chat({ char, profile, answers, history, setHistory, memories, onConversation, voiceOn, setVoiceOn, onBack, onOpenTool, embedded }) {
  ...
  useEffect(() => {
    bargeSpeakingRef.current = handsFree && speaking;
    if (handsFree && speaking) {
      // Give playback a moment to settle before listening for an interruption,
      // so the very start of a reply can't itself be mistaken for the person.
