...
import {
  ...
  Shield, Eye, EyeOff, User, Megaphone, Youtube, ExternalLink, Radio, Paperclip, MessageCircle, Share2, Flame, HelpCircle, Plus, Settings as SettingsIcon,
} from "lucide-react";
...
function SafetyTool({ onBack }) {
  ...
  return (
    <>
      ...
      <div style={{ fontWeight: 700, margin: "22px 2px 4px" }}>Detox &amp; rehab — Western Sydney</div>
...
/* ---------- hub / dashboard ---------- */
function Hub({ profile, plan, progress, saveProgress, journalCount, voiceOn, setVoiceOn, onOpenChat, onOpenJournal, onOpenToolkit, onOpenNotifications, onOpenCoordinator, onOpenSettings, onReset, isAdmin, authEnabled, onOpenAdmin, onOpenProfile, onSignOut, session }) {
  const { speak, stop, speaking } = useVoice(voiceOn);
  ...
  return (
    <>
      ...
      <button onClick={onOpenCoordinator} style={{ width: "100%", background: T.card, borderRadius: 20, padding: 16,
        boxShadow: T.soft, border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, textAlign: "left", marginTop: 10 }}>
        <div style={{ width: 44, height: 44, borderRadius: 14, background: "#dceee2", display: "grid", placeItems: "center", position: "relative" }}>
          <MessageCircle size={20} color="#2c7d50" />
          {unreadCoord > 0 && (
            <span style={{ position: "absolute", top: -4, right: -4, minWidth: 18, height: 18, borderRadius: 999,
              background: "#e5484d", color: "#fff", fontSize: 11, fontWeight: 700, display: "grid", placeItems: "center", padding: "0 4px" }}>{unreadCoord}</span>
          )}
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 700 }}>Message Juan</div>
          <div style={{ fontSize: 13, color: T.sub }}>{unreadCoord > 0 ? `${unreadCoord} reply from Juan` : "Private message to your coordinator"}</div>
        </div>
        <ChevronRight size={20} color={T.sub} />
      </button>
...
/* ---------- chat ---------- */
function Chat({ char, profile, answers, history, setHistory, memories, onConversation, voiceOn, setVoiceOn, onBack, onOpenTool }) {
