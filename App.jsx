...
function Onboarding({ profile, saveProfile, answers, saveAnswers, savePlan, voiceOn, onDone }) {
  ...
  const finish = async () => {
    ...
    let plan = null;
    try {
      const sys = `${CHARS.carlos.system}\nYou are creating a gentle, genuinely personalised 8-week resilience plan from the person's setup answers. Use the specifics they gave — what's weighing on them, the areas they picked, how they cope, what's helped before, their own goal, and the pace they want — so the plan feels made for them, not generic. Match the intensity to their chosen pace and how they're doing. If they picked heavier struggles, keep early weeks small and stabilising. Weave in their existing strengths and support. Respond with ONLY valid JSON, no markdown, in this exact shape:
{"summary": "2-3 warm sentences to the person that reference their actual situation and goal", "weeks":[{"n":1,"focus":"short focus title","steps":["small concrete step","small concrete step"]}, ... 8 weeks]}`;
...
function ProgramPage({ profile, plan, progress, saveProgress, journalCount, onOpenChat, onOpenJournal, onBack }) {
  ...
  const currentWeek = Math.min(weeks.length || 1, 1 + weeks.filter((w) => (w.steps || []).every((_, si) => progress[`w${w.n}s${si}`])).length);
