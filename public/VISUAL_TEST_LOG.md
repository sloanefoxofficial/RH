...
## 27 August 2026 — Portrait layout repair

The supplied phone screenshot showed three concrete failures: the field sat behind the controls, the HUD panel covered the hero, and depth scaling enlarged the hero beyond the visible frame. The first repair attempt also exposed a boot-order bug where the new preview flag was read before declaration, leaving the loader at 0%; that declaration order is now corrected.

The deterministic portrait preview now uses a 390×219 16:9 combat field pinned near the top, a reserved upper HUD band, a portrait hero scale cap of 0.34, and compact touch zones fixed to the bottom corners. The live preview loads and visibly separates the field, HUD, full-body hero, D-pad, and action buttons. The user’s physical phone layout still requires final confirmation on the deployed page, but the code path now matches the required separation rather than placing controls over the lane.

## 27 August 2026 — Portrait separation and wide regression

The portrait preview now separates the HUD, playfield, full-body hero area, D-pad, and action buttons. A subsequent wide regression check revealed that the current shared display compositor makes the hero appear too small in the wide scene. This is a separate regression from the original portrait overlap and must be corrected before delivery: portrait needs a capped hero, while desktop needs the original readable arcade scale.

## 27 August 2026 — Portrait control-coordinate correction

The measured portrait preview now uses the same 390×844 coordinate space for the simulated stage, canvas, HUD, D-pad, and action zone. The combat field is `390×219` at `top:120px`; the HUD occupies the reserved top band; the D-pad and action zone are bottom-anchored inside the phone stage. The player’s portrait depth scale is capped at `0.50`, preventing the near-lane frame from enlarging beyond the field while keeping the full character readable.

The ordinary portrait preview was also checked without a boss-arena inspection state. It showed the same separated layout during active play and no touch-zone overlap with the field.
