...
locked to **sloanefox.official@gmail.com**. Nobody else can get admin, no matter what.
...
## 4b. Create the profiles table (for the user profile panel)

So people can save a preferred name, pronouns, bio, photo, private contact details, and a password.
...
-- Each person can only read and write THEIR OWN profile row.
-- This is what keeps contact details strictly private — no other user, and
-- no admin, can read someone else's row.
...
**sloanefox.official@gmail.com**, and the green **shield (Admin)** appears in the hub header.
...
- Admin isn't a password — it's tied to **one specific signed-in identity**. The app checks: *is the
  logged-in email exactly `sloanefox.official@gmail.com`?* If yes → admin. If not → normal user.
