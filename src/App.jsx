  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (data.session) await loadIsAdmin(data.session.user.id);
      } catch {}
      setAuthChecked(true);
      try {
        const r = supabase.auth.onAuthStateChange(async (_e, s) => {
          setSession(s || null);
          if (s) await loadIsAdmin(s.user.id); else setIsAdmin(false);
        });
      ...
      <div style={{ paddingTop: 8 }}>
        ...
        <div className="rh-in" style={{ background: T.card, borderRadius: 22, padding: 18, boxShadow: T.soft, margin: "18px 0" }}>
          ...
          <input type="password" value={pw} onChange={(e) => setPw(e.target.value)} placeholder="Password"
            ...
            onKeyDown={(e) => e.key === "Enter" && withEmail()} style={inputStyle} />
...
/* ---------- hub / dashboard ---------- */
function Hub({ profile, plan, progress, saveProgress, journalCount, voiceOn, setVoiceOn, onOpenChat, onOpenJournal, onOpenToolkit, onReset, isAdmin, authEnabled, onOpenAdmin, onSignOut }) {
  const { speak, stop, speaking } = useVoice(voiceOn);
